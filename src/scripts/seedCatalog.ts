...
const seedCatalog = async () => {
  ...
  if (!dbUri) throw new Error("DB_URI is required to seed the catalog.");

  try {
    await mongoose.connect(dbUri);
  } catch (error) {
    const fallbackUri = getAtlasFallbackUri(dbUri);
  ...
  await Product.insertMany(
    products.map(([name, category, brand, price, stock, description, photo]) => ({
      name,
      description,
      price,
      stock,
